...
namespace UnitTests
{
    ...
    public unsafe class Class1
    {
        ...
        public void Debug_unsafe()
        {
            ...
            input.bonuses = input.bonuses.Concat(new[] {new RequestInput.BonusData {position = V.Get(22, 22).ToRealCoords(config.width), type = BonusType.N}}).ToArray();

            state.SetInput(config, input);

            var unsafeState = new UnsafeState();
            unsafeState.SetInput(input);

            Console.Out.WriteLine(state.Print());
            Console.Out.WriteLine(unsafeState.Print());

            state.NextTurn(new[] {Direction.Up, Direction.Up}, false);
            var capture = new UnsafeCapture();
            capture.Init();

            unsafeState.ApplyCommand(0, UnsafePlayer.DIR_UP);
            //unsafeState.ApplyCommand(1, UnsafePlayer.DIR_UP);
            unsafeState.NextTurn(&capture, null);
        ...
        public void Debug_visio()
        {
            ...
            var uai = new UnsafeMinimaxAi(2);
            var ustate = new UnsafeState();
            ustate.SetInput(input, "3");

            //var command = uai.GetCommand(&ustate, 0, new TestingTimeManager(100000), new Random(773189632));

            var timeManager = new TimeManager(config);
            timeManager.RequestStarted(1, 0, 0);
            var command = uai.GetCommand(&ustate, 0, timeManager, new Random(773189632));
            ...
            timeManager.RequestStarted(1, 0, 0);
            command = ai.GetCommand(state, 0, timeManager, new Random(773189632));
